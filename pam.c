...
static void
init_env(struct passwd *pw) {
  // CAUTION: use setenv(3) or putenv(3)
...
login(const char *username, const char *password, pid_t*child_pid) {
  ...
  *child_pid = fork();
  if (*child_pid == 0) {
    chdir(pw->pw_dir);
    execl(pw->pw_shell, pw->pw_shell, "-c", "exec /bin/sh .xsession", NULL);
    printf("Failed to execute .xsession");
    exit(1);
  }
